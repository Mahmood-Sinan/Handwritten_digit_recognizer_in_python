...
# loss, accuracy = model.evaluate(x_test, y_test) #testing model with testing data

# print(loss)
# print(accuracy)

image_number = 0
while os.path.isfile(f"digits/{image_number}.png"): #opening the image file
    try:
        img = cv2.imread(f"digits/{image_number}.png")[:,:,0] #getting the image file
        img = np.invert(np.array([img])) #making the image white on black and making it into an array
